...
Next, we run a forward modeling of the total-field anomaly aiming at adjusting the cylinder parameters that are not defined by the RTP anomaly: the thickness $ dz $, the total-magnetization intensity $m_{0}$, and depth to the top $z_{0}$ which yield a preliminary fit of the observed data.
This step does not require such rigor since $ dz $, $ m_0 $, and $ z_0 $ are ultimately defined at the final iteration of the inversion.
...
After that, the ranges for the total-magnetization intensity $m_{0}$ and depth to the top $z_{0}$ forming a grid of $m_0 \times z_0$ can be defined from the cylinder of the preliminary fit.
All inversions will start with a vertical cylinder, nevertheless, their initial guesses will be different  because the total-magnetization intensity $m_{0}$ 
and depth to the top $z_{0}$ will change for each point of the grid $m_0 \times z_0$.
