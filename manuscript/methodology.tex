...
\end{enumerate}
%1) Iteration k = 0: input g o , R, A, p a , p min , p max , μ a , μ r , and
%an initial approximation p 0 satisfying the inequality con-
%straints. For all k, set W (k) equal to the identity matrix.
%2) Obtain p̂ † (k) = f ( p̂ (k) ) using equation (14).
%3) Obtain ∆ p̂ † (k) at the kth iteration by solving equation (14)
%using Marquardt’s (1963) method.
%4) Compute: p̂ † (k+1) = p̂ † (k) + ∆ p̂ † (k) .
%5) Obtain p̂ (k+1) using equation (5).
%6) Test for algorithm convergence. Evaluate 
