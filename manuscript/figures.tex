...
\begin{figure}
    ...
    \caption{Schematic representation the (a) total-filed anomaly (gray surface) produced by 
    (b) a 3-D anomalous source (dark gray volume). The interpretation model in (b) consists 
    of a set of $L$ vertical, juxtaposed prisms (represented in light gray) along the vertical 
    direction.}
    \label{fig:obs}
\end{figure}
...
\begin{figure}
    ...
    \caption{Polygonal cross-section of the 
    $k$th vertical prism described by $V$ vertices (white dots) with polar coordinates 
    ($r^k_j$ , $\theta ^k_j$), $j = 1, \dots, V$, $k = 1, \dots, L$ , referred to an arbitrary origin $O^k$ (grey dot) with horizontal Cartesian coordinates ($x_0^k$ , $y_0^k$), $k = 1, \dots, L$ , (black dot).}
...
\begin{figure}
	...
	total-magnetization intensities ($ m_0 $). 
	The red triangle represents the $m_0$ and $z_0$ of the true source. 
	The white diamond indicates the estimated pair ($ m_0, z_0 $) that produces the smallest value of $\Gamma (\mathbf{p})$.}
...
\begin{figure}
    ...
    height of the observation points and the topographhy,
    respectively. They are referred to the WGS84 ellipsoid and, for convenience, 
    we have subtracted $ 800 $ m from their values.
